...
app.post('/api/ai/chat', async (req, res) => {
  ...
  const rawHistory = Array.isArray(body.messages) ? body.messages.slice(-12) : [];
  // 兼容独立 ZEN AI 页面：如果前端没有单独传 content，自动取最后一条用户消息。
  const lastUserMessage = [...rawHistory].reverse().find(item => {
    const role = String(item?.role || '').toLowerCase();
    return role === 'user' && String(item?.content || '').trim();
  });
...
`).trim();
  ...
  const messages = [
    ...
    ...history.map(item => {
      ...
      const role = declaredRole === 'assistant' || declaredRole === 'system'
        ? declaredRole
        : (item?.isBot ? 'assistant' : 'user');
      return {
        role,
  ...
  try {
    ...
    // 兼容不同上游返回格式。
    // 常见：OpenAI Chat Completions / Responses，以及部分兼容网关。
    const rawAnswer =
    ...
    const answer = Array.isArray(rawAnswer)
      ? rawAnswer.map(item => typeof item === 'string' ? item : (item?.text || item?.content || '')).join('')
      : String(rawAnswer || '');

    const cleanAnswer = answer.trim();

    if (!cleanAnswer) {
      console.error('[AI] upstream returned no usable answer:', raw.slice(0, 1500));
    ...
    const aiMessage = {
      ...
      name: AI_BOT_NAME,
      content: cleanAnswer.slice(0, 4000),
      date: new Date().toISOString(),
      isBot: true,
      botId: 'zen-ai',
    ...
    // 独立 ZEN AI 页面不会写入公共聊天室，避免两边串台。
    // 旧版聊天室请求未传 source 时仍保持原来的公共聊天行为。
    const source = String(body.source || 'chat').toLowerCase();
    ...
    // 同时返回 reply/content 和完整 message，兼容新旧前端。
    res.status(200).json({
      ok: true,
      reply: cleanAnswer.slice(0, 4000),
      content: cleanAnswer.slice(0, 4000),
      message: aiMessage,
